...
dir_case = 'Results_Sim_2024_05_09_wKurt';
...
n_sim = length(corr_z_eps1_vec)*length(corr_z_eps2_vec)*length(eps_kurt_vec)*length(T_vec)*length(p_vec)
...
fH = figure;
h = bar(categorical(max(eps_skew_vec)^2+2+eps_kurt_vec),squeeze(rej_freq_allsim(i_z1,i_z2,:,:,i_p))', 'FaceColor','flat');                     
