...
dir_case = 'ResultsMC\Results_wKurt_TwoSynthProxy';
...
corr_z_eps1_vec  = [0.7];
corr_z_eps2_vec  = -[0 0.2];
% corr_z_eps1_vec  = [0.5; 0.7; 0.9];
% corr_z_eps2_vec  = -[0.5, 0.2, 0];
p_vec           = [0];
eps_skew_vec    = [0;1;2];
eps_kurt_vec    = [0;5;10];
...
n_sim = length(corr_z_eps1_vec)*length(corr_z_eps2_vec)*length(eps_skew_vec)*length(eps_kurt_vec)*length(T_vec)
i_sim = 0;

eta_allsim          = NaN(length(corr_z_eps1_vec),length(corr_z_eps2_vec),length(eps_skew_vec),length(eps_kurt_vec)*length(T_vec),length(p_vec));
rej_freq_allsim     = NaN(length(corr_z_eps1_vec),length(corr_z_eps2_vec),length(eps_skew_vec),length(eps_kurt_vec)*length(T_vec),length(p_vec));
...
for i_z1 = 1:length(corr_z_eps1_vec)
    ...
    for i_z2 = 1:length(corr_z_eps2_vec)
        ...
        assert(func_to_min(psi) < 0.001)
        for i_eps_skew = 1: length(eps_skew_vec)
            eps_skew = eps_skew_vec(i_eps_skew);
            for i_eps_kurt = 1: length(eps_kurt_vec)
                eps_kurt = max(eps_skew_vec)^2+2+eps_kurt_vec(i_eps_kurt);
                    ...
                    for i_p = 1:length(p_vec)
                        ...
                        parfor i_rep = 1:nrep
                            ...
                            elseif strcmp(test_type, 'two_step')
                                thetastart = theta_z;
                                % [J_statistic,p_value,theta_est] = EstAndJTest_03_wKurt_testsascha(U_hat,z_used,thetastart  );
                                % [J_statistic,p_value,theta_est] = EstAndJTest_03(U_hat,z_used,thetastart );
                                [J_statistic,p_value,theta_est] = EstAndJTest_03_wKurt(U_hat,z_used,thetastart  );
                            end
                        ...
                        rej_freq_allsim(i_z1,i_z2,i_eps_skew,i_eps_kurt,i_t,i_p)    = squeeze(squeeze(mean(rej_freq_all,1)));
...
save(filename , 'rej_freq_allsim',...
    ...
    'p_vec','eps_skew_vec','eps_kurt_vec','const',...
